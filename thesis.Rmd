...
\begin{figure}[h]
	...
	\caption{Approximated density of $\hat{\theta}$ for an equidependence skew t - t factor copula model with $\beta = 1.5$, $S = 25000$ and standard normal distributed marginals. Each simulation is based on $1000$ Monte-Carlo replications.}	
	\label{f:montecarlo}  
\end{figure}
...
\end{aligned}
\end{equation}
For all variations of $N$ and $T$, $S = 25000$ was chosen. Each simulation was repeated $1000$ times. The bias and standard deviation of $\hat{\beta}$ was approximated by using the empirical average and standard deviation of all $1000$ simulations.
...
\end{aligned}
\end{equation}
The $21$ observable variables were partitioned in $3$ groups of equal size. Therefore the parameters to be estimated reduced from $0.5*N*(N-1) = 210$ to just $2M = 6$. As in the first example the marginal distributed are iid standard normal. We chose $T = 1500$, $S = 25 \times T$ and a breakpoint at $t = 1000$. Before the break, $\theta_0 = (0, 1, 1, 0, 1, 1)$ and after the break $\theta_1 = (1.5, 1, 1, 1.5, 1, 1)$. Thus, only the intra- and interdependence for the first group increases from $0$ to $1.5$.

Figure \ref{f:breaktest} shows the result of the break test for a single recursive run of the simulation for $t = 300, \dots, 1500$. Both the moments and copula based versions are shown together with the estimated critical value based on $2000$ bootstrap replications.
...
\begin{tabular}{rlll}
  ...
  \bottomrule
\end{tabular}
\caption{Estimation results for the bloc-equidependence factor copula model before and after the breakpoint and for the full dataset. Standard errors in paranthesis (estimated with $ B = 2000$ bootstrap samples).}
\end{table}

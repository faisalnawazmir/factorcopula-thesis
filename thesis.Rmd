...
\textcite[p. 691ff]{Patton2013} showed that under a set of assumptions, the SMM estimator is weakly consistent and asymptotically normal distributed:
\begin{align}
\begin{split}
\hat{\bm{\theta}} \sim N(\bm{\theta_0}, \frac{1}{1/T+1/S}\bm{\Omega}) \text{ for }T, S \rightarrow \infty,
\end{split}
\end{align}
...
The matrix $\bm{W}$ is a positive definite weight matrix and if the efficient weight matrix $\bm{W} = \bm{\Sigma}^{-1}$ is used the aymptotic variance simplifies to $\bm{\Omega} = \bm{(G'WG)^{-1}}$. 

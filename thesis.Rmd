...
\end{split}
\end{align}
The coefficients measure the probability that extreme large (small) values occur in one variable, given extreme large (small) values in the other variable. In contrast to other common dependence measures, the coefficients of upper and lower tail dependency are defined on the interval $[0, 1]$. Since the limit only exists theoretically and not for observable data, one usually calculates upper and lower \emph{quantile} dependency for some values of $q$ close to $0$ for lower and close to $1$ for upper quantile dependence \parencite[62-63]{Joe2015}. The sample versions of these measures can be obtained by replacing the marginal distributions with the EDF and using the definition of the empirical copula \parencite[176]{Nelsen1999}:
\begin{equation}

...
\textcite{Joe2013} and \textcite{Joe2015} also have model named "factor copula" but they use a different appraoch.
...
The parameters of the factor model are collected in a parameter vector $\bm{\theta} = (\beta_{11}, \dots, \beta_{i1}, \dots \beta_{ik}, \bm{\gamma_{Z_1}}', \dots, \bm{\gamma_{Z_K}}', \bm{\gamma_\epsilon}')'$. It consists of all linear coefficients and the distributional parmaters of the error term and the laten variables. The number of latent variables $K$ and the distribution functions $F_{Z_1}, \dots, F_{Z_k}, F_\epsilon$ are hyper-parameters of the model which have to be chosen prior to the estimation.\footnote{\textcite[p. 143ff]{Patton2017} provide a heuristic of finding the number of latent variables by analyzing so called \emph{scree-plots}: Ordered eigenvalues from the sample rank-correlation matrix of the data.}

The artifical set of variables $\bm{X}$ is only used to construct the factor copula function $C_U(u_1, \dots, u_N)$. The parameters of the factor strucutre are choosen in such a way that the resulting copula function fits the copula of the observable data $\bm{Y}$. The copula function can then be used to model the joint distribution of the observable variables such that $F_{Y}(y_1, \dots, y_n) = C_U(F_{Y_1}(y_1),\dots, F_{Y_N}(y_N))$. Once the factor copula function is determined, the artificial variables and its marginal distributions $F_{X_i}(x)$ are of no interest. Using matrix notation, the model can be summarized in the following set of equations:
\begin{align}
...
To model the joint probability $F_Y(\bm{y})$, a two-stage estimation can be applied: First, the marginal distributions $\hat{F}_{Y_i}$ are estimated parametrically or non-parametrically, e.g. by using some parametric model or the empirical distribution function. Second, the factor structure for the copula function is fitted to the data by finding the optimal $\hat{\bm{\theta}}$. Usually a closed form of the factor copula does not exist. Therefore, one has to rely on simulation based estimation methods as described in section \ref{s:SMM}. 

This aproach allows for a variety of different dependence structures and can be applied to high dimensional data. An upper bound for the number of parameters $P = |\bm{\theta}|$ to be estimated is given by the size of the factor matrix and the number of additional free distributional paramters and it holds that $P \leq (NK + |\bm{\gamma_{Z_1}}| + \dots + |\bm{\gamma_{Z_K}}| + |\bm{\gamma_\epsilon}|)$. To reduce the number of parameters, \textcite{Patton2017} present two restrictions on $\bm{\beta}$: the restrictive \emph{equidependence} and the less restrictive \emph{block-equidependence} model. 

...
In the following section, the time series model as presented in the previous section \ref{s:dynamiccopula} and the factor copula model as presented in section \ref{s:factorcopula} are considered and an estimation procedure known as Simulated Method of Moments (SMM) is presented. 

For copulas for which the copula function is known in closed form, standard Maximum Likelihood (ML) estimation can be performed. Given the time series model as described by \eqref{eq:model} multistage maximum likelihood can be applied for estimating the first level conditional means and variances, the marginal distribution of the residuals and the copula. If parametrized models for the marginal distributions are used, it is crucuial that all models are correctly specified, otherwise the estimated copula parameters will be biased parencite[694]{Patton2013}. \textcite[127]{Chen2006} showed that, if the EDF is used for estimating the marginal distribution of the residuals, the first level estimates do not affect the asympotic distribution of the semiparametric ML estimator of the copula parameters.  

Since the factor copula implied by \eqref{eq:factorcopula} is not given in closed form approximating the Likelihood function is difficult and computationally demanding \parencite[694]{Patton2013}. On ther other hand it is easy and computationally cheap to simulate many values from the factor model. Therefore, an alternative approach is used here which is based on the Generalized Methods of Moments (GMM). As shown by \textcite{Chen2006} for the semiparametric ML estimator, the SMM estimator is also not affected by the first level dynamic estimates \parencite[692]{Patton2013} of the time series model in \eqref{eq:model}. 

SMM and GMM estimation is based on the method of moments. For many statistical parametric model, theoretical population moment conditions as functions of the model parameters the random variables involved can be derived. For estimation, the moments in these conditions can then be replaced by their respective sample moments \textcite[6-7]{Hall2005}. 

Formally the moment conditions take the form
\begin{equation}
m(\theta) = E(f(\bm{Y_t}, \bm{\theta})) = 0,
\end{equation}
where $f$ is a vector of functions. For estimation, the moments are replaced by their sample analogs $\hat{m}(\bm{theta}) = \frac{1}{T}\sum_{t = 1}^Tf(\bm{Y_t}, \bm{\theta})$ and then solved for $\hat{\bm{\theta}}$. If more moment conditions than parameters are available no feasable solution exists. Instead, one minimizes the weighted sqared distance to the origin. The GMM estimator is given by:
\begin{equation}
\hat{\bm{\theta}} = \arg \min Q(\bm{\theta}) = \hat{m}'W\hat{m},
\end{equation}

If the DGP is known up to the model parameters one can calculate the moments based on simulated data and compare these moments to the observable sample moments. This method is known as Simultaed method of Moments (SMM) \parencite[342-347]{Hall2005}.

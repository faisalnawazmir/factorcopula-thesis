...
dat$group <- factor(dat$group, levels = 1:4, labels = c("skew t - t", "exp - normal", "t - t", "normal"))

library(ggplot2)
library(ggthemes)
ggplot(data.frame(dat), aes(x = X1, y = X2)) +
  geom_point(alpha = 0.8, col = "indianred3") +
